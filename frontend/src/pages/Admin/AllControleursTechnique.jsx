...
const AllControleursTechnique = () => {
  ...
  const fetchControleurs = async () => {
    setLoading(true);
    try {
      const res = await axiosClient.get("/api/utilisateurs/role/controleur technique");
      setControleurs(res.data.data);
    } catch (err) {
      setMessage("Erreur lors du chargement des contrôleurs technique.");
    } finally {
      setLoading(false);
    }
  };
  ...
  const filtered = controleurs.filter((c) => {
  ...
  const handleDesactiver = async (id) => {
    if (!window.confirm("Désactiver ce contrôleur technique ?")) return;
    try {
      await axiosClient.put(`/api/utilisateurs/${id}/desactiver`);
      setControleurs(controleurs.filter((c) => c.id_utilisateur !== id));
      setMessage("Contrôleur technique désactivé.");
    } catch (err) {
  ...
  const handleModifier = (id) => {
    navigate(`/admin/controleurs-technique/modifier/${id}`);
  };

  return (
    <AdminLayout>
      <div className="p-8">
        <h2 className="text-2xl font-bold mb-6">Tous les Contrôleurs Technique</h2>
        {message && <div className="mb-4 text-red-600">{message}</div>}

        <div className="mb-4 flex justify-between items-center">
          <button
            onClick={() => navigate('/admin/controleurs-technique/ajouter')}
            className="bg-green-600 hover:bg-green-700 text-white font-semibold px-4 py-2 rounded-md shadow mr-4"
          >
            Ajouter Contrôleur Technique
          </button>
          ...
          <table className="min-w-full text-sm">
            ...
            <tbody>
              {loading ? (
                ...
                <tr>
                  <td colSpan={7} className="text-center p-4">Aucun contrôleur technique trouvé.</td>
                </tr>
              ) : (
                filtered.map((c) => (
...
export default AllControleursTechnique; 