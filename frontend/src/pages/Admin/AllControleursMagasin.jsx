...
const AllControleursMagasin = () => {
  ...
  const fetchControleurs = async () => {
    setLoading(true);
    try {
      const res = await axiosClient.get("/api/utilisateurs/role/controleur de magasin");
      setControleurs(res.data.data);
    } catch (err) {
      setMessage("Erreur lors du chargement des contrôleurs de magasin.");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleModifier = (id) => {
    navigate(`/admin/controleurs-magasin/modifier/${id}`);
  };

  return (
    <AdminLayout>
      <div className="p-8">
        <h2 className="text-2xl font-bold mb-6">Tous les Contrôleurs de Magasin</h2>
        {message && <div className="mb-4 text-red-600">{message}</div>}

        <div className="mb-4 flex justify-between items-center">
          <button
            onClick={() => navigate('/admin/controleurs-magasin/ajouter')}
            className="bg-green-600 hover:bg-green-700 text-white font-semibold px-4 py-2 rounded-md shadow mr-4"
          >
            Ajouter Contrôleur de Magasin
          </button>
          ...
          <table className="min-w-full text-sm">
            ...
            <tbody>
              {loading ? (
                ...
                <tr>
                  <td colSpan={7} className="text-center p-4">Aucun contrôleur de magasin trouvé.</td>
                </tr>
              ) : (
                filtered.map((c) => (
...
export default AllControleursMagasin; 